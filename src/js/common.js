$.fn.extend({
     ...
     * @param {Object} [list] 复选框集合
     */
    selectAllParam: function (list) {
        $(this).bind("click", function () {
            if ($(this).attr("checked") == true || $(this).attr("checked") == "checked") {

                $(list).each(function () {
                    if ($(this).attr("data-score") === "" && $(this).attr("data-id") !== "") {
        ...
        $(list).live("click", function () {
            ...
            if ($(this).attr("data-score") !== "") {
                ...
                common.layer.msg("已经存在成绩，无法批量批阅！");
            }
            if ($(this).attr("data-id") === "") {
                $(this).removeAttr("checked");
                common.layer.msg("学生未提交，无法批量批阅！");
            }
            if ($(list).not(":checked").length > 0) {
                $(btn).removeAttr("checked");
            } else {
...
// do the last row
    ...
    bannermover: function () {
        ...
        var indwidth = (listcr.width() + 5) * (listcr.length);
